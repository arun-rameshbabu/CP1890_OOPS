...
def create_movie_entries(conn):
    ...
    movie_collection = (('Spiritted Away', 2005, 125, get_category_id(conn, 'Animation')),
...
def get_movie_by_category(conn, category_id):
    cur = conn.cursor()
    query = f"SELECT rowid, * FROM movies WHERE category = '{category_id}'"
    cur.execute(query)
    result = cur.fetchall()
    movie_list = []
    for row in result:
...
def add_movie(conn, movie_obj):
    ...
    cur.execute(query, (movie_obj.name, movie_obj.year, movie_obj.minutes, movie_obj.category))
    conn.commit()
...
if __name__ == '__main__':
    ...
    # print(get_movie_by_category(conn, 1))
